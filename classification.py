...
class CustomModelforSequenceClassification(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        if self.type == "full":
            ...
            prefix_expanded = self.prefix.unsqueeze(0).expand(input_ids.size(0), -1, -1)
            # get the input embeddings
            input_embeddings = self.model.embeddings(input_ids)
            # print("Shape of inputs_embeddings ", input_embeddings.size()) # 64 x 128 x 768
            inputs_embeds = torch.cat((prefix_expanded, input_embeddings), 1)
            ...
            # print("Shape of inputs_embeds ", inputs_embeds.size()) # 64 x 256 x 768
            ...
            # print("Shape of attention maks before adding prefix mask: ", attention_mask.size())
            prefix_mask = torch.ones(input_ids.size(0), self.prefix.size(0)).to("cuda")
            attention_mask = torch.cat((prefix_mask, attention_mask), dim = 1)
            # print("Shape of attention maks after adding prefix mask: ", attention_mask.size())
...
def train(mymodel, num_epochs, train_dataloader, validation_dataloader, test_dataloder, device, lr, model_name):
    ...
    if mymodel.type == "head":
        ...
        head_parameters = mymodel.classifier.parameters()
